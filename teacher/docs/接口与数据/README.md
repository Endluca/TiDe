...
2. [教师端积分与课程读取对照表](教师端积分与课程读取对照表.md)
3. [TIT 成长系统数据库表结构](数据库表结构.md)
...
- G01 读取 Self-intro／TESOL 两项真实状态，并在站内完成 61 题、Essay 确认和完成证明提交；G04 在站内完成备课确认、设备网络检测和首课画面检查。G02、G05–G08 的视频／测试只使用阔知 iframe；G09 同样只允许阔知执行，但映射发布前保持内容待配置。
...
- `教师端积分与课程读取对照表.md`：教师总览和逐课视图字段、查询与展示边界。
- `数据库表结构.md`：世文 PostgreSQL 当前 24 张业务表及 1 张迁移元数据表。
