...
| 类别 | 主要表 | 说明 |
|---|---|---|
| 执行配置 | `task_execution_versions` / `task_step_definitions` / `task_validation_rules` / `task_template_files` | 只保存步骤、观看限制、题目/清单/上传要求和验证规则，不复制共享静态内容 |
...
- G01 从最新 `public.teacher_metric_snapshots` 读取 `is_self_introduce/is_cpl_tesol`，并结合教师端保存的 61 题结果、Essay 完成确认和完成证明；五项全部满足后才能可信完成。
