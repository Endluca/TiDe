...
共享目录重排时，`task_execution_versions.task_code` 按稳定 `shared_template_row_id` 迁移；执行版本 ID、assignment ID、步骤／规则 key、题库 key、媒体版本和对象存储路径保持不变。展示编码与这些技术身份必须分开，避免既有进度、视频续播和审计记录失联。
...
| 题库配置 | `task_quiz_banks` | 保存按任务和版本导入的题目、标准答案、通过线和来源；前端只接收已发布题目与选项 |
| 专项结果 | `quiz_*` / `checklist_*` / `device_check_*` | 保存答案、清单和设备检查结果，不写入共享 assignment |
...
- G01 按教师 ID 从一师一行的 `public.teacher_source_wide` 读取
  `is_self_introduce/is_cpl_tesol`，并结合教师端保存的 61 题结果、Essay 完成确认和
  完成证明；五项全部满足后才能可信完成。任一外部字段为 `NULL` 时进入数据不可用状态。
