...
3. 视频心跳、清单、提交、图片和文件等过程数据只保存在 `tide`；阔知题目、答案和作答不进入 TIDE。
...
当前按老师 `camp_day` 展示已开放的 G01–G09 共 9 个任务，并继续显示历史阶段任务；G04 是“首课准备”，只含拍照 AI 检查和课件准备确认，隐藏 G00 不生成入口。个性化任务只展示任务触发中心使用真实业务数据创建的 assignment。教师端不创建共享任务，也不使用前端 `mockAssigned` 代替真实分配。任务详情 Why 读取 `task_assignments.why` 并附加 `evidence_snapshot` 的教师安全投影；How、完成标准、完成收益分别读取模板 `payload.how_summary / completion_standard / benefit`，不得以本地执行配置覆盖。
...
| 类别 | 主要表 | 说明 |
|---|---|---|
| 执行配置 | `task_execution_versions` / `task_step_definitions` / `task_validation_rules` | 只保存步骤、观看限制、题目/清单/上传要求和验证规则，不复制共享静态内容 |
| 执行配置 | `task_execution_versions` / `task_step_definitions` / `task_validation_rules` | 只保存步骤、观看限制、清单／上传要求和验证规则，不复制共享静态内容 |
...
- G01 按教师 ID 从一师一行的 `public.teacher_source_wide` 只读取
  `is_cpl_tesol`，并结合阔知课程 407 的考试完成结果、Essay 完成确认和完成证明；四项全部满足后才能可信完成。`is_cpl_tesol` 为 `NULL` 时进入数据不可用状态。`is_self_introduce` 可保留为教师资料来源事实，但不进入 G01 读模型或完成判定。
...
| 领域 | 主要表 | 安全要求 |
|---|---|---|
| 文件 | `file_objects` / `file_upload_intents` / `task_submission_files` / `file_access_events` | 私有存储、SHA-256/MIME/大小校验、提交附件关联和按当前老师鉴权 |
