...
-- Reverse only the two fields changed by 0033. Stable G01 execution,
-- assignment and progress identities remain untouched in both directions.
DO $$
BEGIN
    IF to_regclass('public.task_templates') IS NULL
       ...
       OR to_regclass('tide.task_step_progress') IS NULL THEN
        RAISE EXCEPTION
            'shared task tables and Tide execution tables are required before migration 0033 down';
    END IF;
END
$$;
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION
            'operations G01 stable template G01:v1 is not the approved published row for migration 0033 down';
    END IF;
    ...
    IF execution_id IS NULL THEN
        ...
        END IF;
        RAISE NOTICE
            'migration 0033 down left the empty G01 execution unchanged';
        RETURN;
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'G01 execution identity is not an approved stable shape for migration 0033 down';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'G01 external-status rule is missing or has an unreviewed shape before migration 0033 down';
    END IF;
END
$$;
...
BEGIN
    ...
    IF updated_count = 0 THEN
        IF (
            ...
            RAISE EXCEPTION
                'migration 0033 down did not update exactly one approved G01 external-status rule';
        END IF;
    ELSIF updated_count <> 1 THEN
        RAISE EXCEPTION
            'migration 0033 down updated % G01 external-status rules instead of one',
            updated_count;
    END IF;
END
$$;
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'migration 0033 down changed the G01 execution row';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'migration 0033 down changed G01 assignments';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'migration 0033 down changed G01 progress';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'migration 0033 down changed G01 step definitions';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'migration 0033 down changed another G01 validation rule';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'migration 0033 down changed immutable G01 external-status rule fields';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'migration 0033 down did not restore exactly one pre-0033 G01 rule';
    END IF;
END
$$;
