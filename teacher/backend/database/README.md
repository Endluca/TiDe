...
| `0014_teacher_photo_processing` | 保存首课摄像头画面的四项真检、后台图片处理、原图与最终私有资源追溯链 |
| `0015_teacher_photo_filter_strength` | 允许首课画面处理使用完整滤镜强度 |
...
| `0022_performance_job_leases` | 为图片处理和后台任务增加租约、重试与认领索引 |
...
| `scripts/apply.sh` | 本地幂等升级至 0025，导入题库，并应用当前 Seed/本地权限 |
| `scripts/apply-company-test.sh` | 初始化公司测试库的 `tide` Schema、G01–G09 与 5 个已发布个性化任务码族执行配置和受限应用账号；不写 Mock Seed 或共享模板 |
| `scripts/apply-production.sh` | 仅执行生产结构迁移；先校验运营 rev38 权威目录，再使用账本、SHA-256 和 PostgreSQL advisory lock 升级至 0025 |
| `scripts/test-production-migrator.sh` | 在两个临时 PostgreSQL 数据库显式构造 rev38 共享目录，验证 fresh、managed upgrade、0022–0025、execution ID 原位保留、checksum、工单函数 owner 和生产连接保护 |
...
`ALLOW_MOCK_SEED=true` 才会执行。`apply.sh` 当前升级至 `0025`，但仍是本地
Mock 入口，不能用于公司或生产库。
...
- `apply-company-test.sh` 会补齐 `0001–0025`，导入数据库题库，并以只读模式读取已发布的 G01–G09 与 5 个个性化任务码族写入教师端执行配置；运行前同样要求运营 rev38 权威目录已就绪。
