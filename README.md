...
- 当前代码迁移 head 为 public `20260811_54_g04_remove_device_check` 与 teacher
  ...
  `20260811_51_g01_tesol_only` / `0033_g01_tesol_only` 与 rev54/0037；远程 G04 仍为历史三模块形状。`tit_growth_test_v2`
  已于 2026-08-10 受控升级；重建前旧库封存为
  `tit_growth_test_v2_pre0030_20260810`，仅保留 DBA 回滚连接。当前结构中
  `teacher_source_wide` 为 CSV 教师 61 列加 2 个可空教师资料状态字段（G01 只消费 TESOL），
  ...
  合并投诉规则来源表并原位瘦身两张积分账户表；rev49 删除两个可从投诉原始行恢复的学习字段和未独立维护的会话活跃时间；rev50 更新 G04 历史三模块文案；rev51 将 G01 上游状态收窄为仅 TESOL；rev54 将 G04 收敛为拍照审核与课件准备两个模块；teacher 0029 删除 6 张空置无消费者表和
...
联合部署门禁要求按 `public 46 → teacher 0028 → public 50 → teacher 0032 → public 54 → teacher 0037` 分阶段迁移，最终到达
public `20260811_54_g04_remove_device_check` 和教师端 `0037_g04_remove_device_check`，并同时通过
