...
- 当前代码迁移 head 为 public `20260811_55_source_wide_v12` 与 teacher
  `0037_g04_remove_device_check`，最终 teacher canonical 账本为 32 条，其中
  `20260811_51_g01_tesol_only` / `0033_g01_tesol_only`
  将 G01 收窄为 TESOL-only，rev54/0037 将 G04 收窄为照片审核与课件准备两模块。
  交接测试库仍为 public `20260810_50_g04_sections` 与 teacher
  `0032_first_login_onboarding`，仍是精确 30 条 canonical 账本，尚未应用
  `20260811_51_g01_tesol_only` / `0033_g01_tesol_only`、rev54/0037 与 rev55；远程 G04 仍为历史三模块形状。`tit_growth_test_v2`
  已于 2026-08-10 受控升级；重建前旧库封存为
  ...
  合并投诉规则来源表并原位瘦身两张积分账户表；rev49 删除两个可从投诉原始行恢复的学习字段和未独立维护的会话活跃时间；rev50 更新 G04 历史三模块文案；rev51 将 G01 上游状态收窄为仅 TESOL；rev54 将 G04 收敛为拍照审核与课件准备两个模块；rev55 将教师源表收敛为确认的 55 列；teacher 0029 删除 6 张空置无消费者表和
  5 个被当前实现替代的分析视图，0030 继续删除固定为私有的文件可见性列与无消费者的孤儿函数，0031/0032/0033 分别完成 G04 历史三模块、账号引导事实与 G01 TESOL-only 校验规则，0037 删除 G04 当前设备检测步骤并保留历史进度。旧库 `tit_growth_test` 未原地改造。
...
联合部署门禁要求按 `public 46 → teacher 0028 → public 50 → teacher 0032 → public 54 → teacher 0037 → public 55` 分阶段迁移，最终到达
public `20260811_55_source_wide_v12` 和教师端 `0037_g04_remove_device_check`，并同时通过
固定提交源码中的精确 `G01–G09` 标题/分值预检和目标数据库契约探针。
其中 public 54 阶段包含 rev51 G01 TESOL-only，teacher 37 阶段包含 0033 G01 规则迁移，public 55 最后收敛教师源字段。
教师端未到 0037、最终 canonical 账本不是精确 32 条、目录缺项或语义错误都会失败关闭；在 public 47 及之后的空库直接
