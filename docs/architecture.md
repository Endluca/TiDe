...
Gaea 的运营端交付形态把根目录 React 构建产物复制到 FastAPI 的 `app/static` 包目录，
由同一个 Python 服务在 8010 端口提供页面、`/assets/*` 和 `/api/*`。这个合并只消除运营
前端的独立 Nginx/容器，不改变逻辑进程边界：Alembic 仍是发布前独立作业，固定任务积分
结算 Worker 仍保持独立单实例，`teacher/` 教师端仍独立部署。
...
- 当前运营 API 的公开读写路径已直接使用 PostgreSQL；输出重试、关联通知、审计和 Outbox
  在同一事务完成，共享测试库双 Worker 冲突验证通过。固定任务积分结算进程仍保持单实例；
  真实触达 Worker、外部日更、生产监控和恢复演练尚未完成。
- Gaea 多模块骨架已包含 Node 构建、Python 包内静态资源与 FastAPI 同源托管模块，以及
  heartbeat 健康检查的单实例积分结算模块；该配置只证明源码具备构建入口，不代表内部
  镜像构建、Gaea 环境、Ingress、迁移作业或生产切流已经完成。
