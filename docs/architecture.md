...
Gaea 测试交付使用一个项目、一个镜像和一个 Pod。镜像内仍保留四个逻辑进程：FastAPI
在 `8010` 同源提供运营 React 与 API，教师 Nginx 在 `8080` 提供教师 React 并代理到
Pod 内 `3000` 的 NestJS，积分 Worker 不暴露端口。Gaea 分别把运营、教师域名绑定到
`8010/8080`。s6 负责进程生命周期，聚合健康检查同时覆盖两端 HTTP 与 Worker heartbeat。
整个 Pod 必须固定单副本；Alembic 和教师端 migration 仍是发布前独立作业，数据库角色与
业务写权限不会因为共享镜像而合并。更新策略必须是 Recreate 或显式先缩到 0，防止滚动
更新临时并存两个 Worker。这个受控 TEST 形态不提供进程级秘密隔离：同 UID 进程可接触
容器级环境变量，生产信任边界成立前仍应拆容器或 Pod。
