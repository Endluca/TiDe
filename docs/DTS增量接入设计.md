...
1. Gaea 正式接入使用已在国内 PRE 成功消费的官方 Kafka Java Client 1.0.0 负责 Kafka 传输；Python 使用 `fastavro` 和官方 Avro schema 解码原始 payload，并继续拥有国内 HMAC、PostgreSQL 事务、数据库 checkpoint、脏键和宽表投影。`kafka-python` 仅保留为显式回退诊断。
   默认 Java transport 保持单条 in-flight：Java 输出 EVENT，Python 数据库事务成功后返回 DURABLE_ACK，Java 才同步提交 `offset + 1` 并返回 COMMITTED。DB checkpoint 存在时必须精确 seek 该 `next_offset`，Kafka committed 领先 DB 时失败关闭；DB 无 checkpoint 时才按 `TIT_DTS_START_AT` 定位并忽略旧 committed。第 16 条中 TCP、Metadata v5 cap 和两个 `TIT_DTS_KAFKA_STARTUP_*` 参数仅描述 `TIT_DTS_TRANSPORT=kafka_python` 回退模式，不传给正式 Java transport。
...
17. 回退 TCP 探针只建立并立即关闭 socket；两种启动门禁都只读取 metadata/offset，不写接入账本/镜像/脏键/宽表，也不提交消费组 offset。启动成功只证明当前具备开始消费的条件，不等于已经消费到一条消息；实际接入必须另看首个成功 heartbeat、事件账本、数据库 checkpoint 和消费组位点。Java 稳态只有收到 Python durable ACK 才提交 Kafka offset；明确可重试的 Java 断线/超时会清除健康证据、关闭当轮 DB/Java 资源并在同一 PID 内按数据库 checkpoint 重跑完整启动门禁，永久错误仍失败关闭。
