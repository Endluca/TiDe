"""add derived is_perfect to the teacher lesson read view

Revision ID: 20260728_27_lesson_perfect
Revises: 20260727_26_irrev_qual
...
revision: str = "20260728_27_lesson_perfect"
down_revision: Union[str, None] = "20260727_26_irrev_qual"
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None


def _create_teacher_lesson_view(*, include_is_perfect: bool) -> None:
    perfect_column = (
        ",\n            perfect_fact.is_perfect" if include_is_perfect else ""
    )
    perfect_json = (
        """
    ...
    op.execute(
        ...
        BEGIN
            ...
            IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tit_teacher_crud') THEN
                ...
                TO tit_teacher_crud;
            END IF;
            IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tide_business_app') THEN
                GRANT SELECT ON TABLE public.teacher_lesson_score_current
...
def upgrade() -> None:
    ...
    _create_teacher_lesson_view(include_is_perfect=True)
    _grant_view_read()
...
def downgrade() -> None:
    if op.get_bind().dialect.name != "postgresql":
        return
    op.execute("DROP VIEW IF EXISTS public.teacher_lesson_score_current")
    _create_teacher_lesson_view(include_is_perfect=False)
    _grant_view_read()
