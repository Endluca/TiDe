...
- `TIT_DTS_PROJECTION_MODE=direct` 是 2026-08-19 新增的显式、未发布替代路径：保留 checkpoint 幂等、投诉分类参考字典和国内 HMAC 指纹契约，在同一事务内由 before/after 直接写两张宽表；只有教师/课程 INSERT 创建主行，无法命中宽表的历史 UPDATE、DELETE 和子事件记为 ignored 并推进 checkpoint。默认 `queued` 不变。direct 的 slot 为首次 `非 on→on` 单向累加，关闭/删除不回减，完整规则和切换前提见 `docs/DTS事件直接投影规则.md`。
