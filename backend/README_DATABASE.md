...
- 当前宽表中的完课、Peak、好评、收藏、15 日复约、迟到、早退、`perfect_cnt` 和 `peak_slot_cnt` 等按 `REAL / DERIVED_REAL` 追溯。15 日复约只保留事实、不计分；当前可靠性分读取 `perfect_cnt × 4 + peak_completed_cnt × 2`，课堂质量当前不加分。
