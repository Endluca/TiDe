...
`public 46 → teacher 0028 → public 50 → teacher 0032 → public 54 → teacher 0037 → public 55 → release public 56 → teacher 0038 → release public 57 → teacher 0040 → teacher 0041 → public 59–65 → teacher 0042 → public 66–99 → teacher 0043`
完成跨 Schema 的 release 内容链与 DTS v2/教师执行目录迁移，详见
...
总控脚本不会运行 teacher 初始化器、Mock/内容 Seed、发布 Gaea 或重启服务。到达 public 99 /
teacher 0043 后，仍需使用包含 `tit_teacher_crud` 凭据的另一份 Git 外配置运行
...
- `TIT_DTS_PROJECTION_MODE=direct` 是 2026-08-19 新增的显式替代路径：保留 checkpoint 幂等、投诉分类参考字典和国内 HMAC 指纹契约，在同一事务内由 before/after 直接写两张宽表；教师/课程 INSERT，或当前完整镜像满足 cohort/课程范围的 UPDATE，可以创建主行，缺失主行的 DELETE、范围外 UPDATE 和子事件记为 ignored 并推进 checkpoint。默认 `queued` 不变。direct 的 slot 为首次 `非 on→on` 单向累加，关闭/删除不回减，完整规则和切换前提见 `docs/DTS事件直接投影规则.md`。
