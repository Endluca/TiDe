...
from app.config_models import (  # noqa: E402
...
def upgrade_score_config_v9(
    *,
    service: ConfigService | None = None,
    app_env: str | None = None,
    creator_actor_id: str = "system:score-v9-upgrade-creator",
    publisher_actor_id: str = "system:score-v9-upgrade-publisher",
) -> dict:
    """Publish the current reliability, classroom-quality and Gold policy."""
    ...
    if creator_actor_id == publisher_actor_id:
        raise ValueError(
            "The high-impact score config requires different creator and "
            "publisher actors."
        )
    ...
    try:
        normalized_current = ScoreGraduationConfig.model_validate(
            current
    ...
    if normalized_current == locked_default:
        return {"status": "SKIPPED_ALREADY_V9", "policy_version": "v9"}

    draft = resolved_service.create_draft(
        ConfigKey.SCORE_GRADUATION,
        actor_id=creator_actor_id,
        payload=deepcopy(SCORE_POLICY_V9_PAYLOAD),
    )
    ...
    if not validation["valid"]:
        raise RuntimeError(
            f"Default v9 score config failed validation: {validation['errors']}"
        )
...
def main() -> int:
    try:
        result = upgrade_score_config_v9()
