...
def upgrade_score_config_v10(
    *,
    service: ConfigService | None = None,
    app_env: str | None = None,
    creator_actor_id: str = "system:score-v10-upgrade-creator",
    publisher_actor_id: str = "system:score-v10-upgrade-publisher",
) -> dict:
    """Publish the nine-task completion gate after the G02/G05 merge."""
    ...
    if normalized_current == locked_default:
        return {"status": "SKIPPED_ALREADY_V10", "policy_version": "v10"}
    ...
    if not validation["valid"]:
        raise RuntimeError(
            f"Default v10 score config failed validation: {validation['errors']}"
        )
...
def main() -> int:
    try:
        result = upgrade_score_config_v10()
