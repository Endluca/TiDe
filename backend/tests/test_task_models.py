...
def test_english_template_rejects_chinese_teacher_facing_why() -> None:
    with pytest.raises(
        ValidationError,
        match="why_template must be English teacher-facing copy",
    ):
        CreateTaskTemplateRequest.model_validate(
            _request(why_template="该任务由课堂信号触发。")
        )
